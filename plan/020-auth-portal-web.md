...
| `/it/login` | Admins | Login nội bộ — tách hoàn toàn |
...
│
└── it/
    ├── +layout.svelte            → IT shell (stripped, dark)
    └── login/
        ├── +page.svelte          → Admin login form
        └── +page.server.ts       → Form action: login → redirect /it/dashboard
```
...
| `/it/login` | `/it/dashboard` | Stay, hiện error |

### Guard

- `/b2b/**` (trừ `/b2b` landing và `/b2b/login`): yêu cầu role `ORGANIZER` — redirect `/b2b/login` nếu không có token.
- `/it/**` (trừ `/it/login`): yêu cầu role `ADMIN` — redirect `/it/login` nếu không có token.
...
| `AuthCard.svelte` | `lib/components/auth/` | Centered card wrapper (B2B, IT) |
...
| `/it/login` page bg | `ink` (#303841) |
| `/it/login` input | `form-input` dark variant |
...
- [ ] `/it/login` login thành công → redirect `/it/dashboard`.
- [ ] `/it/login` login với role ≠ `ADMIN` → "Access denied", session không được tạo.
- [ ] `/it/**` (trừ `/it/login`) guard: redirect `/it/login` nếu không có token hoặc role ≠ `ADMIN`.
- [ ] Token refresh tự động trong `hooks.server.ts`.
- [ ] Tất cả error states hiển thị đúng (wrong password, rate limit).
- [ ] `/it/login` không có nav, footer, register, OAuth, forgot password.
