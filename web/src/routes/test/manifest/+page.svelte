<script lang="ts">
	...
	const mockVenue = { id: '018f4e1a-0006-4000-8000-000000000001', name: 'Nhà hát Lớn Hà Nội' }
	...
	const mockManifest = {
		...
		totalCapacity: 404,
		objects: [
			{ type: 'stage', x: 0, y: 80, width: 160, height: 680, text: 'STAGE' }
		]
	}

	const mockLevels = [
		{ id: 'L1', description: 'Main Floor', color: '#3B82F6' }
	]

	const mockSections = [
		{ id: 'M100001-SEC-VIP', name: 'Front', color: '#F59E0B', levelId: 'L1', type: 'RS' as const },
		{ id: 'M100001-SEC-PREM', name: 'Center', color: '#3B82F6', levelId: 'L1', type: 'RS' as const },
		{ id: 'M100001-SEC-STD', name: 'Rear', color: '#EC4899', levelId: 'L1', type: 'RS' as const },
		{ id: 'M100001-SEC-GA', name: 'GA Lobby', color: '#10B981', levelId: 'L1', type: 'GA' as const }
	]

	const mockPriceLevels = [
		{ id: 'PL-VIP', description: 'VIP – 2.500.000₫', color: '#F59E0B' },
		{ id: 'PL-PREM', description: 'Premium – 1.500.000₫', color: '#3B82F6' },
		{ id: 'PL-STD', description: 'Standard – 800.000₫', color: '#EC4899' }
	]

	const mockGaSections = [
		{
			id: 'M100001-SEC-GA',
			name: 'GA Lobby',
			levelId: 'L1',
			...
			color: '#10B981',
			polygon: [],
			x: 620, y: 80, width: 160, height: 680
		}
	]

	function buildRsSections() {
		// Stage: y=80, h=680 → y=80..760
		// 3 sections stacked vertically: Left, Center, Right
		const sections: any[] = [
			{ id: 'M100001-SEC-VIP', name: 'Left', color: '#F59E0B', levelId: 'L1', useHull: false,
				uiData: { x: 180, y: 80, width: 400, height: 200 }, rows: [] as any[],
				rowStart: 1, rowEnd: 15, seatsPerRow: 8, priceLevel: 'PL-VIP', yStart: 100 },
			{ id: 'M100001-SEC-PREM', name: 'Center', color: '#3B82F6', levelId: 'L1', useHull: false,
				uiData: { x: 180, y: 280, width: 400, height: 240 }, rows: [] as any[],
				rowStart: 1, rowEnd: 15, seatsPerRow: 10, priceLevel: 'PL-PREM', yStart: 300 },
			{ id: 'M100001-SEC-STD', name: 'Right', color: '#EC4899', levelId: 'L1', useHull: false,
				uiData: { x: 180, y: 520, width: 400, height: 200 }, rows: [] as any[],
				rowStart: 1, rowEnd: 15, seatsPerRow: 8, priceLevel: 'PL-STD', yStart: 540 }
		]
		const rowXStart = 200, rowSpacing = 26, seatSpacing = 22
		for (const sec of sections) {
			for (let r = sec.rowStart; r <= sec.rowEnd; r++) {
				const rowName = String.fromCharCode(64 + r)
				const rowId = `${sec.id}-row-${rowName}`
				const rowX = rowXStart + (r - sec.rowStart) * rowSpacing
				const row: any = { id: rowId, sectionId: sec.id, name: rowName, seats: [] }
				for (let s = 1; s <= sec.seatsPerRow; s++) {
					row.seats.push({
						id: `${rowId}-s${String(s).padStart(3, '0')}`,
						rowId, name: String(s).padStart(3, '0'),
						positionX: rowX,
						positionY: sec.yStart + (s - 1) * seatSpacing,
						status: 'AVAILABLE',
						priceLevelId: sec.priceLevel,
						sectionId: sec.id
					})
				}
				// Mark a few seats as unavailable
				if (row.seats.length > 0) row.seats[0].status = 'UNAVAILABLE'
				if (row.seats.length > 3) row.seats[3].status = 'UNAVAILABLE'
				sec.rows.push(row)
			}
		}
		return sections
	...
	let venue = $state(mockVenue)
	let manifestIdVal = $state(mockManifest.id)
	let manifestDescVal = $state(mockManifest.description)
	let manifestCapVal = $state(mockManifest.totalCapacity)
	let levels = $state<any[]>(mockLevels)
	let sections = $state<any[]>(mockSections)
	let priceLevels = $state<any[]>(mockPriceLevels)
	let gaSections = $state<any[]>(mockGaSections)
	let rsSections = $state<any[]>(buildRsSections())
	let layoutObjects = $state<any[]>(mockManifest.objects)
	...
	let brushPriceLevelId = $state(priceLevels[0]?.id || 'STANDARD')
	...
	let priceLevelBasePrices = $state<Record<string, number>>({
		'PL-VIP': 2500000, 'PL-PREM': 1500000, 'PL-STD': 800000
	})
	...
	let viewportX = $state(0), viewportY = $state(0)
	let viewportW = $state(800), viewportH = $state(600)
	let currentScale = $state(1)
	let fitScale = $state(1)

	function getSeatRadius(_sc: number) { return 3 }
	function _showSeats(sc: number) { return sc >= fitScale * 1.5 }
	function _secProgress(sc: number) {
		const start = fitScale * 1.4, end = fitScale * 2.2
	...
	const financialInfo = $derived.by(() => {
		let totalSeats = 0, totalRevenue = 0
	...
	onMount(() => {
		...
		requestAnimationFrame(async () => {
			...
			selectionRect = new Konva.Rect({
				fill: 'rgba(59,130,246,0.1)', stroke: '#3b82f6', strokeWidth: 1.5, visible: false
			})
	...
	function fitToView() {
		if (!stage) return
		const pad = 80
		const sw = stage.width() - pad * 2, sh = stage.height() - pad * 2
	...
	function zoomIn() {
		if (!stage) return
		const os = stage.scaleX(), ns = Math.min(os * 1.3, 5)
	...
	function zoomOut() {
		if (!stage) return
		const os = stage.scaleX(), ns = Math.max(os / 1.3, 0.1)
		const cx = stage.width() / 2, cy = stage.height() / 2
		const mp = { x: (cx - stage.x()) / os, y: (cy - stage.y()) / os }
		stage.scale({ x: ns, y: ns })
		stage.x(cx - mp.x * ns)
	...
	function getSectionColor(section: any): string {
		...
		for (const [id, c] of Object.entries(cnt))
			if (c > max) { max = c; dom = id }
		if (dom) { const s = sections.find((s) => s.id === dom); if (s?.color) return s.color }
		...
		for (const [id, c] of Object.entries(pc))
			if (c > pmax) { pmax = c; pdom = id }
		if (pdom) { const p = priceLevels.find((p) => p.id === pdom); if (p?.color) return p.color }
		return '#3B82F6'
	}
	...
	function drawSeatingMap() {
		...
		layoutObjects.forEach((obj, idx) => {
			const ox = obj.x ?? 100, oy = obj.y ?? 100, ow = obj.width ?? 200, oh = obj.height ?? 100
			const isSel = selectedObjectId === idx
			const g = new Konva.Group({ x: ox, y: oy, draggable: false, id: 'obj-' + idx })
			if (obj.type === 'stage') {
				g.add(new Konva.Rect({ width: ow, height: oh, fill: '#334155', stroke: isSel ? '#F59E0B' : '#64748B', strokeWidth: isSel ? 3 : 2, cornerRadius: 8 }))
				g.add(new Konva.Text({ text: (obj.text || 'STAGE').toUpperCase(), fontSize: Math.max(12, Math.min(ow, oh) * 0.2), fontStyle: 'bold', fill: '#F8FAFC', align: 'center', verticalAlign: 'middle', width: ow, height: oh }))
			} else if (obj.type === 'label') {
				const t = new Konva.Text({ text: obj.text || 'Label', fontSize: obj.fontSize || 14, fontStyle: 'bold', fill: isSel ? '#F59E0B' : obj.color || '#0F172A', align: 'center', verticalAlign: 'middle' })
				g.add(new Konva.Rect({ width: t.width() + 16, height: t.height() + 16, x: -8, y: -8, fill: 'transparent', stroke: isSel ? '#F59E0B' : 'transparent', strokeWidth: 1.5, cornerRadius: 4 }))
				g.add(t)
			} else {
				const fill = obj.color || '#CBD5E1', op = obj.opacity !== undefined ? Number(obj.opacity) : 0.5
				if (obj.shape === 'circle') {
					const r = obj.radius || Math.min(ow, oh) / 2
					g.add(new Konva.Circle({ x: r, y: r, radius: r, fill, opacity: op, stroke: isSel ? '#F59E0B' : '#94A3B8', strokeWidth: isSel ? 3 : 1 }))
				} else {
					g.add(new Konva.Rect({ width: ow, height: oh, fill, opacity: op, stroke: isSel ? '#F59E0B' : '#94A3B8', strokeWidth: isSel ? 3 : 1, cornerRadius: obj.cornerRadius || 4 }))
				}
			}
			g.on('click tap', (e: any) => { e.cancelBubble = true; selectedObjectId = idx; selectedGaSectionId = ''; selectedRsSectionId = ''; drawSeatingMap() })
			layer.add(g)
		})

		gaSections.forEach((ga, idx) => {
			const x = ga.x || 50 + idx * 220, y = ga.y || 50, gw = ga.width || 200, gh = ga.height || 100
			const isSel = selectedGaSectionId === ga.id
			const g = new Konva.Group({ x, y, draggable: false, id: ga.id })
			g.add(new Konva.Rect({ width: gw, height: gh, fill: isSel ? '#F1F5F9' : '#F8FAFC', stroke: isSel ? '#0F172A' : '#CBD5E1', strokeWidth: isSel ? 2 : 1 }))
			const gaLabel = (ga.name || ga.id).toUpperCase()
			g.add(new Konva.Text({ text: gaLabel, fontSize: Math.max(14, Math.min(gw, gh) * 0.2), fontStyle: 'bold', fill: '#94A3B8', opacity: 0.4, width: gw, height: gh, align: 'center', verticalAlign: 'middle' }))
			g.on('click tap', (e: any) => { e.cancelBubble = true; selectedGaSectionId = ga.id; selectedRsSectionId = ''; selectedObjectId = null; drawSeatingMap() })
			layer.add(g)
		})
		...
		rsSections.forEach((section) => {
			...
			if (section.polygon?.length >= 6) {
				hull = (section.polygon as [number, number][]).flatMap(([px, py]) => [px - boxX, py - boxY])
			...
			const shapeAttrs = {
				...
				strokeWidth: isSel ? 4 : 4, hitStrokeWidth: 0, closed: true
			}
			...
			if (showLabel) {
				const cleanId = (id: string) => id.replace(/[-_]/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase()).trim()
				...
				ag.add(new Konva.Text({
					text: labelText, fontSize, fontStyle: 'bold', fontFamily: 'Inter, system-ui, sans-serif',
					fill: '#4A5568', opacity: Math.max(0.75, 1 - secProgress * 0.35),
					align: 'center', verticalAlign: 'middle', x: 0, y: 0, width: boxW, height: boxH,
			...
			if (showSeats) {
				rows.forEach((row: any, ri: number) => {
					...
					;(row.seats || []).forEach((seat: any, si: number) => {
						...
						const sg = new Konva.Group({ x: sx - boxX, y: sy - boxY, id: seat.id, draggable: activeMode === 'floor-edit' })
						sg.add(new Konva.Circle({ radius: SEAT_R, fill: color, opacity: isFiltered ? 0.15 : 1, stroke: 'transparent', strokeWidth: 0 }))
						if (isSeatSel) {
							const sw = Math.max(1.5 / sc, 1)
							sg.add(new Konva.Circle({ radius: SEAT_R - sw / 2, stroke: '#000000', strokeWidth: sw, listening: false }))
						}
						...
						circle.on('click tap', (e: any) => {
							...
							if (selectionTool === 'seat') {
								if (isRight) { selectedSeatIds = selectedSeatIds.filter((x) => x !== seat.id); drawSeatingMap() }
								return
							}
							let ids: string[] = []
						...
						circle.on('mouseenter', () => { if (activeTool !== 'pan') stage.container().style.cursor = pointer2Cursor })
						circle.on('mouseleave', () => { if (activeTool !== 'pan') stage.container().style.cursor = pointer2Cursor })
						sg.on('dragend', () => {
	...
	function setupCanvasEvents() {
		if (!stage) return
		let isDown = false, brushActive = false
		stage.on('mousedown touchstart', (e: any) => {
			if (e.target === stage) { selectedObjectId = null; selectedGaSectionId = ''; selectedRsSectionId = ''; drawSeatingMap() }
			isDown = true
			const t = e.target
			const isSeat = t?.className === 'Circle' && t?.parent?.id() && !t.parent.id().startsWith('group-') && !t.parent.id().startsWith('obj-')
			if (isSeat) {
				brushActive = true
				const id = t.parent?.id() || t.id()
				if (id && !id.startsWith('group-') && id !== stage.id()) {
					if (activeTool === 'eraser') { removeSeatById(id); drawSeatingMap() }
						...
						const isRight = e.evt?.button === 2 || e.evt?.which === 3
						if (isRight) selectedSeatIds = selectedSeatIds.filter((x) => x !== id)
						else if (isShiftPressed) selectedSeatIds = selectedSeatIds.includes(id) ? selectedSeatIds.filter((x) => x !== id) : [...selectedSeatIds, id]
			...
			if (!pos) return
			const s = stage.scaleX() || 1
			x1 = (pos.x - stage.x()) / s; y1 = (pos.y - stage.y()) / s; x2 = x1; y2 = y1
			selectionRect.x(x1); selectionRect.y(y1); selectionRect.width(0); selectionRect.height(0)
			selectionRect.visible(true); layer.batchDraw()
		})
				...
				if (shape?.className === 'Circle' && !shape.parent?.id()?.startsWith('obj-')) {
					...
					if (id && !id.startsWith('group-') && id !== stage.id()) {
						if (activeTool === 'eraser') { removeSeatById(id); drawSeatingMap() }
							...
							selectedSeatIds = [...selectedSeatIds, id]; drawSeatingMap()
						}
					}
				}
				return
				...
				(section.rows || []).forEach((row: any) =>
					(row.seats || []).forEach((seat: any) => {
						...
						if (sx >= rx && sx <= rx + rw && sy >= ry && sy <= ry + rh) {
							...
							else (section.rows || []).forEach((r: any) => r.seats.forEach((s: any) => sel.push(s.id)))
						}
					})
				)
			)
			selectedSeatIds = sel.length > 0 ? Array.from(new Set([...selectedSeatIds, ...sel])) : selectedSeatIds
			drawSeatingMap()
		})
	}
	...
	function removeSeatById(id: string) {
		...
		rsSections.forEach((section) => {
			...
			section.rows.forEach((row: any) => { if (row.seats) row.seats = row.seats.filter((s: any) => s.id !== id) })
			section.rows = section.rows.filter((row: any) => row.seats?.length > 0)
		})
	}

	function paintSeat(id: string, plId: string) { const f = findSeatById(id); if (f) f.seat.priceLevelId = plId }
	function paintSection(id: string, secId: string) { const f = findSeatById(id); if (f) f.seat.sectionId = secId }

	function deleteSelectedSeats() {
		if (!selectedSeatIds.length) return
		rsSections.forEach((section) => {
			if (!section.rows) return
			section.rows.forEach((row: any) => { if (row.seats) row.seats = row.seats.filter((s: any) => !selectedSeatIds.includes(s.id)) })
	...
	function generateSeatingBlock() {
		if (!selectedRsSectionId) { errorMessage = 'Select a Reserved Seating Area first.'; return }
		...
		for (let ri = 0; ri < blockRowsCount; ri++) {
			...
			let cnt = blockSeatsPerRow, off = 0
			if (blockShape === 'trapezoid') { cnt = blockSeatsPerRow + ri * 2; off = -ri * 16 }
			else if (blockShape === 'diamond') {
				const mid = Math.floor(blockRowsCount / 2), d = Math.abs(ri - mid)
				cnt = Math.max(1, blockSeatsPerRow - d * 2); off = d * 16
			...
			for (let si = 0; si < cnt; si++) {
				...
				newRow.seats.push({
					id: `${rowId}-seat-${num}`, rowId, name: String(num).padStart(3, '0'),
					positionX: 120 + si * 32 + off, positionY: rowY,
					status: 'AVAILABLE', priceLevelId: brushPriceLevelId || null, sectionId: section.id
				})
			}
			rows.push(newRow)
		}
	...
	function setInventoryStatus(status: 'AVAILABLE' | 'UNAVAILABLE') {
		...
		rsSections.forEach((section) =>
			(section.rows || []).forEach((row: any) =>
				(row.seats || []).forEach((seat: any) => { if (selectedSeatIds.includes(seat.id)) seat.status = status })
			)
		)
		rsSections = [...rsSections]
	...
	function assignPriceLevelToSelected(plId: string) {
		...
		rsSections.forEach((section) =>
			(section.rows || []).forEach((row: any) =>
				(row.seats || []).forEach((seat: any) => { if (selectedSeatIds.includes(seat.id)) seat.priceLevelId = plId })
			)
		)
		rsSections = [...rsSections]
	...
	function createLookup(type: 'level' | 'section') {
		...
		if (!code) { errorMessage = 'Code required.'; return }
		const item = { id: code.trim(), description: code.trim(), color }
		if (type === 'level') { levels = [...levels, item]; levelCode = '' }
		else { sections = [...sections, item]; sectionCode = '' }
		saveMessage = `Added ${type}.`; errorMessage = ''
	}
	...
	function runAssistantPrompt() {
		...
		if (q.includes('vip') || q.includes('gold'))
			rsSections.forEach((a) => (a.rows || []).forEach((r: any) => (r.seats || []).forEach((s: any) => {
			...
			if (m) rsSections.forEach((a) => (a.rows || []).forEach((r: any) => {
				if (r.name.toUpperCase() === m[1].toUpperCase()) ids.push(...(r.seats || []).map((s: any) => s.id))
			}))
		} else if (q.includes('all'))
			rsSections.forEach((a) => (a.rows || []).forEach((r: any) => ids.push(...(r.seats || []).map((s: any) => s.id))))
		if (ids.length > 0) { selectedSeatIds = ids; saveMessage = `Selected ${ids.length} seats.`; errorMessage = ''; drawSeatingMap() }
		else errorMessage = `No seats matched '${assistantPrompt}'.`
		assistantPrompt = ''
	}

	function exportLayout(fmt: 'json' | 'csv' | 'svg') {
		let fn = `manifest-${manifestIdVal || 'export'}-${Date.now()}`, content = '', mime = 'text/plain'
		if (fmt === 'json') {
			content = JSON.stringify({ levels, sections, priceLevels, gaSections, rsSections }, null, 2)
			mime = 'application/json'; fn += '.json'
		} else if (fmt === 'csv') {
			const rows = ['SeatID,AreaID,Row,SeatName,PositionX,PositionY,Status,PriceLevelId,SectionId']
			rsSections.forEach((a) => (a.rows || []).forEach((r: any) => (r.seats || []).forEach((s: any) =>
				rows.push([s.id, a.id, r.name, s.name, s.positionX, s.positionY ?? r.positionY, s.status, s.priceLevelId || '', s.sectionId || ''].join(','))
			)))
			content = rows.join('\n'); mime = 'text/csv'; fn += '.csv'
		} else {
			...
			svg += '</svg>'; content = svg; mime = 'image/svg+xml'; fn += '.svg'
		}
...
<div class="flex h-screen w-screen flex-col overflow-hidden bg-slate-50 font-sans select-none">
	<header class="z-50 flex h-12 shrink-0 items-center justify-between border-b border-slate-200 bg-white px-4">
		<div class="flex items-center gap-2">
			<a href="/test/manifest" class="rounded-lg p-1.5 text-slate-400 transition-colors hover:bg-slate-100 hover:text-slate-800" aria-label="Back">
		...
		<div class="flex rounded-lg border border-slate-200 bg-slate-100 p-0.5">
			{#each ['scaling', 'inventory', 'floor-edit'] as mode}
				<button onclick={() => { activeMode = mode as any; selectedSeatIds = [] }}
					class="rounded-md px-3 py-1 text-[10px] font-bold capitalize transition-all {activeMode === mode ? 'bg-white text-slate-900 shadow-xs' : 'text-slate-500 hover:text-slate-800'}">
					{mode === 'floor-edit' ? 'Floor Edit' : mode.charAt(0).toUpperCase() + mode.slice(1)}
				</button>
			{/each}
		</div>
		<div class="flex items-center gap-2">
			{#if saveMessage}<span class="text-[10px] font-bold text-emerald-600">{saveMessage}</span>{/if}
			{#if errorMessage}<span class="text-[10px] font-bold text-rose-600">{errorMessage}</span>{/if}
			<div class="relative">
				<button onclick={() => (showExportDropdown = !showExportDropdown)}
				...
				{#if showExportDropdown}
					<div class="absolute right-0 z-60 mt-1.5 w-44 rounded-lg border border-slate-200 bg-white py-1 shadow-xl">
						<button onclick={() => { exportLayout('json'); showExportDropdown = false }} class="w-full px-3 py-2 text-left text-[10px] font-bold text-slate-700 hover:bg-slate-50">Layout (JSON)</button>
						<button onclick={() => { exportLayout('csv'); showExportDropdown = false }} class="w-full px-3 py-2 text-left text-[10px] font-bold text-slate-700 hover:bg-slate-50">Seats (CSV)</button>
						<button onclick={() => { exportLayout('svg'); showExportDropdown = false }} class="w-full px-3 py-2 text-left text-[10px] font-bold text-slate-700 hover:bg-slate-50">Blueprint (SVG)</button>
					</div>
				{/if}
			</div>
	...
	<div class="relative flex min-h-0 flex-1">
		...
		<aside class="flex w-[280px] shrink-0 flex-col border-r border-slate-200 bg-white">
			<div class="flex items-center justify-between border-b border-slate-200 px-4 py-2.5">
				...
				<button onclick={() => priceLevels.push({ id: 'NEW', description: 'New Level', color: getNextPriceLevelColor() })}
			...
			<div class="flex-1 space-y-px overflow-y-auto px-3 py-2">
				<div class="flex items-center gap-2 rounded-lg px-2.5 py-2 text-[11px]">
					<span class="flex h-3 w-3 shrink-0 items-center justify-center rounded-full border-2 border-slate-300 bg-white"></span>
				...
				{#each priceLevels as pl}
					...
					<div class="flex items-center gap-2 rounded-lg px-2.5 py-2 text-[11px] transition-colors hover:bg-slate-50 {brushPriceLevelId === pl.id ? 'ring-2 ring-slate-300 ring-inset' : ''}"
						onclick={() => { brushPriceLevelId = pl.id; activeTool = 'brush' }} role="button">
						<span class="h-3 w-3 shrink-0 rounded-full border border-white/50" style="background:{pl.color}"></span>
						<div class="flex flex-1 flex-col gap-0.5">
							...
							<div class="flex items-center gap-1 text-[10px] text-slate-400">
								...
								<input type="number" step="0.01" min="0" value={price}
									oninput={(e) => { priceLevelBasePrices = { ...priceLevelBasePrices, [pl.id]: Number((e.target as HTMLInputElement).value) || 0 } }}
			...
			<div class="border-t border-slate-200 bg-slate-50/50 px-4 py-3">
				<h4 class="mb-2 text-[9px] font-black tracking-widest text-slate-400 uppercase">Financial Information</h4>
		...
		<main class="relative flex flex-1 flex-col overflow-hidden bg-[#FAFAFA]">
			<div id="stage-container" bind:this={stageContainer} role="application" aria-label="Seatmap"
				class="relative w-full flex-1 overflow-hidden" oncontextmenu={(e) => e.preventDefault()}></div>

			<!-- Zoom controls -->
			<div class="absolute right-4 bottom-4 z-30 flex flex-col items-center gap-0.5 rounded-xl border border-slate-200/80 bg-white/90 p-0.5 shadow-lg backdrop-blur-md">
				<button type="button" onclick={zoomIn} class="flex h-7 w-7 items-center justify-center rounded-lg text-slate-500 hover:bg-slate-100" title="Zoom In">
					<svg class="h-3.5 w-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 4v16m8-8H4" /></svg>
				</button>
				<button type="button" onclick={zoomOut} class="flex h-7 w-7 items-center justify-center rounded-lg text-slate-500 hover:bg-slate-100" title="Zoom Out">
				...
				<button type="button" onclick={fitToView} class="flex h-7 w-7 items-center justify-center rounded-lg text-slate-500 hover:bg-slate-100" title="Fit to View">
			...
			<!-- Mini-map -->
			<div class="absolute right-16 bottom-4 z-30 flex h-32 w-44 flex-col overflow-hidden rounded-xl border border-slate-200/80 bg-white/90 p-2 shadow-lg backdrop-blur-md">
				<div class="flex items-center justify-between pb-1 text-[8px] font-bold tracking-wider text-slate-400 uppercase">
					<span>Map</span>
					<span class="rounded bg-slate-100 px-1 py-0.5 font-mono text-[7px] text-slate-500">{Math.round(currentScale * 100)}%</span>
				</div>
				<div class="relative w-full flex-1 overflow-hidden rounded-lg border border-slate-200/60 bg-slate-50">
					<button type="button" aria-label="Minimap"
						...
						<svg viewBox="{bounds.minX} {bounds.minY} {bounds.width} {bounds.height}" class="pointer-events-none h-full w-full">
							{#each rsSections as section}
								...
								{#if section.polygon?.length >= 6}
									<polygon points={section.polygon.map((p: [number, number]) => p.join(',')).join(' ')} fill={secColor} fill-opacity="0.9" stroke="#FFFFFF" stroke-opacity="1" stroke-width="2" vector-effect="non-scaling-stroke" />
								{:else if section.uiData}
									<rect x={section.uiData.x} y={section.uiData.y} width={section.uiData.width} height={section.uiData.height} fill={secColor} fill-opacity="0.9" stroke="#FFFFFF" stroke-opacity="1" stroke-width="2" vector-effect="non-scaling-stroke" />
								{/if}
							{/each}
							{#each gaSections as ga}
								<rect x={ga.x || 50} y={ga.y || 50} width={ga.width || 200} height={ga.height || 100} fill="none" stroke="#334155" stroke-opacity="0.35" stroke-width="1.5" rx="3" />
							{/each}
							{#each layoutObjects as obj}
								{#if obj.type === 'stage'}
									<rect x={obj.x ?? 100} y={obj.y ?? 100} width={obj.width ?? 200} height={obj.height ?? 100} fill="#334155" stroke="#64748B" stroke-width="1.5" rx="4" />
								{:else if obj.type === 'label'}
									<rect x={(obj.x ?? 0) - 4} y={(obj.y ?? 0) - 4} width={8} height={8} fill={obj.color || '#0F172A'} stroke={obj.color || '#0F172A'} stroke-opacity="0.5" stroke-width="1" rx="1" />
								{:else}
									<rect x={obj.x ?? 100} y={obj.y ?? 100} width={obj.width ?? 200} height={obj.height ?? 100} fill={obj.color || '#94A3B8'} fill-opacity="0.25" stroke={obj.color || '#64748B'} stroke-opacity="0.7" stroke-width="1.5" rx="3" />
								{/if}
							{/each}
							<rect x={viewportX} y={viewportY} width={viewportW} height={viewportH} fill="rgba(239,68,68,0.06)" stroke="#EF4444" stroke-width="5" stroke-dasharray="8 6" rx="3" />
						</svg>
					</button>
				</div>
			</div>
			...
			{#if selectedSeatIds.length > 0}
				<div class="absolute top-3 left-3 z-30 flex items-center gap-3 rounded-xl border border-slate-200 bg-white/95 px-4 py-2.5 shadow-lg backdrop-blur-md">
					<div class="flex h-8 w-8 items-center justify-center rounded-lg bg-blue-100">
						<svg class="h-4 w-4 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 5v2m0 4v2m0 4v2M5 5a2 2 0 00-2 2v3a2 2 0 110 4v3a2 2 0 002 2h14a2 2 0 002-2v-3a2 2 0 110-4V7a2 2 0 00-2-2H5z" /></svg>
					</div>
					<span class="text-xs font-bold text-slate-800">{selectedSeatIds.length} seats selected</span>
					<div class="flex gap-1">
						<button onclick={() => setInventoryStatus('AVAILABLE')}
							class="rounded-lg border border-emerald-200 bg-emerald-50 px-2 py-1 text-[10px] font-bold text-emerald-700 transition hover:bg-emerald-100">Mark Available</button>
						<button onclick={() => setInventoryStatus('UNAVAILABLE')}
							class="rounded-lg border border-rose-200 bg-rose-50 px-2 py-1 text-[10px] font-bold text-rose-700 transition hover:bg-rose-100">Mark Unavailable</button>
					</div>
					<div class="flex gap-1">
						{#each priceLevels as pl}
							<button onclick={() => assignPriceLevelToSelected(pl.id)}
								class="rounded-lg px-2 py-1 text-[10px] font-bold text-white transition" style="background:{pl.color}">{pl.id}</button>
						{/each}
					</div>
					<button onclick={() => { selectedSeatIds = []; drawSeatingMap() }}
						class="ml-2 rounded-lg border border-slate-200 px-3 py-1.5 text-[10px] font-bold text-slate-600 transition hover:bg-slate-50 hover:text-slate-800">Clear</button>
				</div>
			{/if}

			{#if isSaving}
				<div class="absolute inset-0 z-60 flex flex-col items-center justify-center gap-3 bg-white/70 backdrop-blur-xs">
		...
		<!-- ── Right Tool Palette ── -->
		<aside class="flex w-[56px] shrink-0 flex-col items-center gap-0.5 border-l border-slate-200 bg-white py-3">
			<button onclick={() => { activeTool = 'select'; selectionTool = 'seat' }}
				class="flex w-10 flex-col items-center gap-0.5 rounded-lg px-1 py-2 text-[9px] font-bold transition-colors {activeTool === 'select' && selectionTool === 'seat' ? 'bg-slate-100 text-slate-900' : 'text-slate-400 hover:bg-slate-50 hover:text-slate-700'}">
				<IconPointer2 class="h-4 w-4" />
				Seat
			</button>
			<button onclick={() => { activeTool = 'select'; selectionTool = 'row' }}
				class="flex w-10 flex-col items-center gap-0.5 rounded-lg px-1 py-2 text-[9px] font-bold transition-colors {activeTool === 'select' && selectionTool === 'row' ? 'bg-slate-100 text-slate-900' : 'text-slate-400 hover:bg-slate-50 hover:text-slate-700'}">
				<svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M4 6h16M4 12h16M4 18h16" /></svg>
				Row
			</button>
			<button onclick={() => { activeTool = 'select'; selectionTool = 'section' }}
				class="flex w-10 flex-col items-center gap-0.5 rounded-lg px-1 py-2 text-[9px] font-bold transition-colors {activeTool === 'select' && selectionTool === 'section' ? 'bg-slate-100 text-slate-900' : 'text-slate-400 hover:bg-slate-50 hover:text-slate-700'}">
			...
			<button onclick={() => (showFilterDialog = !showFilterDialog)}
				...
				<svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z" /></svg>
				Filters
			</button>
			<button onclick={() => (showAssistantDialog = !showAssistantDialog)}
				class="flex w-10 flex-col items-center gap-0.5 rounded-lg px-1 py-2 text-[9px] font-bold text-slate-400 transition-colors hover:bg-slate-50 hover:text-slate-700">
...
{#if showFilterDialog}
	<div class="fixed top-40 right-24 z-50 w-60 rounded-2xl border border-slate-200 bg-white/95 p-4 shadow-xl backdrop-blur-md">
		...
		<div class="space-y-2.5 text-xs font-semibold text-slate-600">
			...
			<label class="flex cursor-pointer items-center gap-2 opacity-40" title="Field not in API model">
				<input type="checkbox" bind:checked={onlyAccessible} class="h-3.5 w-3.5 rounded" disabled /><span>♿ Accessible Only <span class="text-[9px] text-slate-400">(n/a)</span></span>
			</label>
			<label class="flex cursor-pointer items-center gap-2 opacity-40" title="Field not in API model">
				<input type="checkbox" bind:checked={onlyObstructed} class="h-3.5 w-3.5 rounded" disabled /><span>⚠️ Obstructed Only <span class="text-[9px] text-slate-400">(n/a)</span></span>
			</label>
		</div>
	</div>
{/if}
...
{#if showAssistantDialog}
	<div class="fixed top-56 right-24 z-50 w-72 rounded-2xl border border-slate-200 bg-white/95 p-4 shadow-xl backdrop-blur-md">
		<div class="mb-3 flex items-center justify-between border-b border-slate-100 pb-2">
		...
		<form onsubmit={(e) => { e.preventDefault(); runAssistantPrompt() }} class="space-y-3">
			...
			<div class="flex gap-2">
				<input type="text" bind:value={assistantPrompt} placeholder="Prompt..."
					class="flex-1 rounded-lg border border-slate-200 px-2.5 py-1.5 text-xs font-semibold outline-none focus:border-slate-400" />
				<button type="submit" class="rounded-lg bg-slate-900 px-3 py-1.5 text-xs font-bold text-white hover:bg-black">Run</button>
			</div>
		</form>
	</div>
{/if}
