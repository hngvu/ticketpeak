...
<div class="mx-auto max-w-[1600px] px-4 py-8 md:px-8">
	...
	<main class="col-span-12 md:col-span-8 rounded-md border border-hairline bg-white p-5">
		{#if data.query && data.suggestions.length > 0}
			<div class="mb-6">
				<div class="mb-2 h-1 w-8 rounded-full bg-ink"></div>
				<h2 class="mb-4 text-lg font-bold tracking-[0.1em] uppercase text-ink">Top Suggestions</h2>
				<HorizontalScroll>
					{#each data.suggestions as suggestion}
						...
						>
							...
							<div>
								...
								<span class="block text-sm font-semibold text-ink truncate">{suggestion.name}</span>
							</div>
						</a>
					{/each}
				</HorizontalScroll>
		...
		<div class="mb-6 border-b border-hairline pb-4">
			<div class="mb-2 h-1 w-8 rounded-full bg-ink"></div>
			<h1 class="flex items-center gap-1 text-lg font-bold tracking-[0.1em] uppercase text-ink">
				<span>Events</span>
				<span class="mx-1.5 font-black text-3xl leading-none tracking-normal">&middot;</span>
				<span class="font-normal tracking-normal">{data.totalResults} Result{data.totalResults !== 1 ? 's' : ''}</span>
			</h1>
		</div>
	...
	<aside class="col-span-12 md:col-span-4 rounded-md border border-hairline bg-white p-5">
		<h3 class="mb-3 text-sm font-bold uppercase text-ink">Advertisement</h3>
