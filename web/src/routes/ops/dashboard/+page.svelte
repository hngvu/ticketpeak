<script lang="ts">
	...
	let countryDropdownContainer = $state<HTMLDivElement | null>(null);

	let cityDropdownOpen = $state(false);
	let citySearchQuery = $state('');
	let cityDropdownContainer = $state<HTMLDivElement | null>(null);
	...
	const citiesList = [
		{ id: 1, name: 'Hà Nội', countryCode: 'VN' },
		{ id: 2, name: 'TP. Hồ Chí Minh', countryCode: 'VN' },
		{ id: 3, name: 'Đà Nẵng', countryCode: 'VN' },
		{ id: 4, name: 'Hải Phòng', countryCode: 'VN' },
		{ id: 5, name: 'Cần Thơ', countryCode: 'VN' },
		{ id: 6, name: 'Nha Trang', countryCode: 'VN' },
		{ id: 7, name: 'Huế', countryCode: 'VN' },
		{ id: 8, name: 'Đà Lạt', countryCode: 'VN' },
		{ id: 9, name: 'Vũng Tàu', countryCode: 'VN' },
		{ id: 10, name: 'Quy Nhơn', countryCode: 'VN' },
		{ id: 11, name: 'New York', countryCode: 'US' },
		{ id: 12, name: 'Los Angeles', countryCode: 'US' },
		{ id: 13, name: 'Chicago', countryCode: 'US' },
		{ id: 14, name: 'Houston', countryCode: 'US' },
		{ id: 15, name: 'Las Vegas', countryCode: 'US' },
		{ id: 16, name: 'Miami', countryCode: 'US' },
		{ id: 17, name: 'Seattle', countryCode: 'US' },
		{ id: 18, name: 'San Francisco', countryCode: 'US' },
		{ id: 19, name: 'Sydney', countryCode: 'AU' },
		{ id: 20, name: 'Melbourne', countryCode: 'AU' },
		{ id: 21, name: 'London', countryCode: 'GB' },
		{ id: 22, name: 'Singapore', countryCode: 'SG' },
		{ id: 23, name: 'Tokyo', countryCode: 'JP' },
		{ id: 24, name: 'Seoul', countryCode: 'KR' },
		{ id: 25, name: 'Bangkok', countryCode: 'TH' }
	];
	...
	const filteredCities = $derived(
		citiesList
			.filter((c) => !orgCountryCode || c.countryCode === orgCountryCode)
	...
	function selectCountry(code: string) {
		...
		countrySearchQuery = '';
		const currentCity = citiesList.find((c) => c.id.toString() === orgCityId);
		if (currentCity && currentCity.countryCode !== code) {
			orgCityId = '';
		}
	}

	function selectCity(id: number) {
		orgCityId = id.toString();
	...
	function handleDocumentClick(event: MouseEvent) {
		...
		if (
			...
			!countryDropdownContainer.contains(target)
		) {
			countryDropdownOpen = false;
		}
		if (cityDropdownOpen && cityDropdownContainer && !cityDropdownContainer.contains(target)) {
			cityDropdownOpen = false;
		}
	}
		...
		>
			...
			<form
				...
				use:enhance={() => {
					return async ({ result, update }) => {
						if (result.type === 'success') {
							addLog('ORG_CREATE', `Organization "${orgName}" created successfully.`);
			...
			>
				...
				<div class="space-y-1">
					...
					{:else if searchedOwner}
						<div
							class="mt-2.5 flex items-center gap-3 rounded-lg border border-[#E4E4E7] bg-[#FAFAFA] p-2.5 select-none"
						>
							{#if searchedOwner.avatarUrl}
								<img
									src={searchedOwner.avatarUrl}
									alt="Owner Avatar"
									class="h-8 w-8 rounded-full border border-[#E4E4E7] bg-white object-cover"
								/>
							{:else}
								<div
									class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-slate-200 text-xs font-bold text-slate-700 uppercase"
								>
							...
							<div class="flex min-w-0 flex-col">
								<span class="truncate text-xs font-bold text-[#111111]">
									{searchedOwner.firstName || ''}
									{searchedOwner.lastName || ''}
								</span>
								<span
									class="mt-0.5 text-[9px] font-semibold tracking-wider text-[#71717A] uppercase"
								>
									Role: {searchedOwner.role} • Status: {searchedOwner.status}
								</span>
							</div>
						</div>
					{/if}
				...
				<div class="space-y-1">
					<label class="text-xs font-semibold text-[#71717A]">Logo Image</label>
					<ImageInput bind:value={orgLogo} name="logoUrl" />
				</div>

				<div class="grid grid-cols-2 gap-4">
					<div class="space-y-1" bind:this={countryDropdownContainer}>
						<label class="text-xs font-semibold text-[#71717A]">Country</label>
						<div class="relative">
							<input type="hidden" name="countryCode" value={orgCountryCode} />
							<button
								type="button"
								onclick={() => {
								...
								class="flex w-full items-center justify-between rounded-lg border border-[#E4E4E7] bg-[#FAFAFA] px-3.5 py-2.5 text-left font-sans text-xs text-[#111111] outline-none focus:border-[#71717A] focus:bg-white"
							>
								<span>
									{countriesList.find((c) => c.code === orgCountryCode)?.name ||
										'Select Country...'}
									{#if orgCountryCode}
										<span class="font-normal text-[#71717A]"> ({orgCountryCode})</span>
									{/if}
								</span>
								<svg
									class="h-4 w-4 text-[#71717A]"
									fill="none"
									viewBox="0 0 24 24"
									stroke="currentColor"
								>
							...
							{#if countryDropdownOpen}
								<div
									class="absolute right-0 left-0 z-50 mt-1 flex max-h-56 flex-col overflow-hidden rounded-lg border border-[#E4E4E7] bg-white shadow-lg"
								>
									<div class="border-b border-[#E4E4E7] bg-[#FAFAFA] p-2">
										<input
											type="text"
											placeholder="Search country..."
											bind:value={countrySearchQuery}
											class="w-full rounded-md border border-[#E4E4E7] bg-white px-2.5 py-1.5 font-sans text-xs text-[#111111] outline-none focus:border-[#71717A]"
											onclick={(e) => e.stopPropagation()}
										/>
									</div>
									<div class="max-h-40 overflow-y-auto">
										{#if filteredCountries.length === 0}
											<div class="px-3.5 py-2 text-xs text-[#71717A] italic">
												No countries found
											</div>
										{:else}
											{#each filteredCountries as c (c.code)}
												<button
													...
													class="flex w-full cursor-pointer items-center justify-between px-3.5 py-2 text-left text-xs text-[#111111] transition-colors hover:bg-[#FAFAFA]"
												>
													<span>{c.name} ({c.code})</span>
													{#if orgCountryCode === c.code}
														<svg
					...
					<div class="space-y-1" bind:this={cityDropdownContainer}>
						<label class="text-xs font-semibold text-[#71717A]">City</label>
						<div class="relative">
							<input type="hidden" name="cityId" value={orgCityId} />
							<button
								type="button"
								onclick={() => {
							...
							>
								<span>
									{citiesList.find((c) => c.id.toString() === orgCityId)?.name || 'Select City...'}
									{#if orgCityId}
										<span class="font-normal text-[#71717A]"> (ID: {orgCityId})</span>
									{/if}
								</span>
								<svg
									class="h-4 w-4 text-[#71717A]"
								...
								>
									...
									<div class="max-h-40 overflow-y-auto">
										{#if filteredCities.length === 0}
											<div class="px-3.5 py-2 text-xs text-[#71717A] italic">No cities found</div>
										{:else}
