<script lang="ts">
	...
	$effect(() => {
		...
		if (api.length > 0) {
			const merged = [...api];
			seedUsers.forEach((s) => {
				if (!merged.some((u) => u.email.toLowerCase() === s.email.toLowerCase())) merged.push(s);
			});
			usersList = merged;
		} else {
			usersList = seedUsers;
	...
	function toggleAll() {
		selected = allSel
			? selected.filter((id) => !rows.some((u) => u.id === id))
			: [...new Set([...selected, ...rows.map((u) => u.id)])];
	}
	...
	function toggleBan(id: string) {
		...
		if (u) u.status = u.status === 'ACTIVE' ? 'BANNED' : 'ACTIVE';
	}
...
<div class="flex min-h-screen flex-col bg-[#F5F6F8]">
	<!-- Page Header -->
	<div class="border-b border-slate-200 bg-white px-8 pt-7 pb-0">
		<div class="mb-5 flex items-start justify-between">
			<div>
				<div
					class="mb-1 flex items-center gap-2 text-[11px] font-semibold tracking-widest text-slate-400 uppercase"
				>
				...
				<p class="mt-0.5 text-[13px] text-slate-500">
					Manage all registered accounts, roles, and access control.
				</p>
			</div>
			<button
				class="flex items-center gap-2 rounded-md bg-slate-900 px-4 py-2.5 text-[13px] font-semibold text-white shadow-sm transition hover:bg-slate-800 active:scale-95"
			>
		...
		<div class="flex gap-8 border-t border-slate-100 pt-4">
			{#each [{ label: 'Total Users', value: stats.total, color: 'text-slate-900' }, { label: 'Active', value: stats.active, color: 'text-emerald-600' }, { label: 'Banned', value: stats.banned, color: 'text-red-500' }, { label: 'Organizers', value: stats.orgs, color: 'text-amber-600' }] as s}
				<div class="pb-4">
	...
	<div class="flex items-center justify-between border-b border-slate-200 bg-white px-8 py-3">
		<div class="flex items-center gap-3">
			...
			<div class="relative">
				...
				<input
					...
					bind:value={search}
					class="h-8 w-72 rounded-md border border-slate-200 bg-white pr-3 pl-9 text-[13px] font-medium text-slate-800 placeholder-slate-400 transition outline-none focus:border-slate-400 focus:ring-2 focus:ring-slate-200"
				/>
			</div>
				...
				{#each [['all', 'All Roles'], ['ADMIN', 'Admin'], ['BUYER', 'Buyer'], ['ORGANIZER', 'Organizer'], ['VENUE_MANAGER', 'Venue Mgr'], ['PROMOTER', 'Promoter']] as [val, lbl]}
					<button
						...
						class="h-8 border-r border-slate-200 px-3 text-[12px] font-semibold transition last:border-r-0
							...
							: 'text-slate-500 first:rounded-l-md last:rounded-r-md hover:bg-slate-50'}"
						>{lbl}</button
					>
				{/each}
			</div>
		...
		<div class="flex items-center gap-3 text-[12px] font-medium text-slate-400">
			...
			{#if selected.length > 0}
				...
				<button
					class="rounded-md border border-red-200 bg-red-50 px-3 py-1 text-[12px] font-semibold text-red-600 transition hover:bg-red-100"
					>Bulk Ban</button
				>
			{/if}
		</div>
			...
			<table class="w-full border-collapse text-left">
				<thead>
					<tr class="border-b border-slate-200 bg-slate-50">
						...
						{#each ['Name', 'Email', 'Role', 'Joined', 'Status', 'Actions'] as col, i}
							<th
								class="px-4 py-3 text-[11px] font-bold tracking-widest text-slate-400 uppercase {i ===
								5
									? 'text-right'
									: ''}"
							>
				...
				<tbody>
					{#each rows as user (user.id)}
						...
						<tr
							class="group border-b border-slate-100 transition-colors last:border-b-0
						...
						>
							...
							<td class="px-4 py-3.5">
								<div class="flex items-center gap-3">
									<div
										class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-slate-100 text-[12px] font-bold text-slate-600"
									>
										{user.name
											.split(' ')
											.map((w) => w[0])
									...
									<span
										class="text-[13px] font-semibold text-slate-900 {isBanned
							...
							<td class="px-4 py-3.5">
								<span
									class="inline-flex items-center rounded-md px-2 py-0.5 text-[11px] font-semibold ring-1 ring-inset {roleMeta.color}"
								>
									{roleMeta.label}
								</span>
							</td>
							...
							<!-- Joined -->
							<td class="px-4 py-3.5">
								<span class="text-[13px] text-slate-500 tabular-nums"
									>{fmtDate(user.registeredAt)}</span
								>
							</td>
							...
							<td class="px-4 py-3.5">
								{#if isBanned}
									<span
										class="inline-flex items-center gap-1.5 rounded-md bg-red-50 px-2.5 py-1 text-[11px] font-bold tracking-wide text-red-600 uppercase ring-1 ring-red-200 ring-inset"
									>
								...
								{:else}
									<span
										class="inline-flex items-center gap-1.5 rounded-md bg-emerald-50 px-2.5 py-1 text-[11px] font-bold tracking-wide text-emerald-700 uppercase ring-1 ring-emerald-200 ring-inset"
									>
			...
			{#if totalPages > 1}
				...
				>
					<span class="text-[12px] text-slate-400">
						Showing {currentPage * PER_PAGE + 1}–{Math.min(
