...
- **Exception Factory Pattern**: Each module defines a single `<Module>Exception` final class in `<module>/exception/` (e.g. `EventException`, `VenueException`, `OfferException`). This class contains one static factory method per distinct error the module can produce. Each factory method encapsulates the HTTP status, error code string, and default message — nothing is specified at the throw site. Services call only the factory method: `throw EventException.notFound()`. Never use `new ApiException(HttpStatus.X, "RAW_STRING", "message")` directly in service code. Never define error code strings outside the module's own `<Module>Exception` class.
- **Conflict Handling**: Programmatically check for unique constraints (e.g. duplicate names, codes) in the service layer using `existsBy...` or similar queries. Throw the module's conflict factory method (e.g. `throw EventException.slugAlreadyExists()`) rather than letting lower-level database constraints bubble up to unhandled 500 errors.
